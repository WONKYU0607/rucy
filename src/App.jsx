...
import { getFirestore, doc, getDoc, setDoc, collection, query, where, getCountFromServer } from 'firebase/firestore'
...
const SKILL_SHEET = [
  ...
  { id: 48, n: 8, h: 200, stage: 5, title: '궁극의 소용돌이', charSeq: [1, 2, 3, 4, 5, 6, 7, 8], cd: 2, dmgMult: 3, aoe: true, rangePx: 220, hitAt: 0.85 },   // 1~4 소용돌이(시트1), 5~8 베기(시트2)
  { id: 47, n: 6, h: 200, stage: 5, title: '회전검객', charSeq: [1, 2, 3, 4, 5, 6], cd: 2, dmgMult: 3, aoe: true, rangePx: 200, hitAt: 0.55 },   // 시트 1~6, 이펙트는 그림에 포함
  { id: 46, n: 5, h: 200, stage: 5, title: '파워 찌르기', charSeq: [1, 2, 3, 4, 5], cd: 2, dmgMult: 3, aoe: true, rangePx: 200, hitAt: 0.55 },   // 시트 2·3·4·5·6 → 1~5, 이펙트는 그림에 포함
  { id: 45, n: 6, h: 200, stage: 5, title: '무한검술', charSeq: [1, 2, 3, 4, 5, 6, 1, 2, 3, 4, 5, 6, 1, 2, 3, 4, 5, 6], cd: 2, dmgMult: 3, aoe: true, rangePx: 200, hitAt: 0.5 },   // 시트 1·2·3·5·7·8 → 1~6, 3바퀴
  { id: 44, n: 7, h: 200, stage: 5, title: '번개 토네이도', charSeq: [1, 2, 3, 4, 5], fx: { type: 'strike', frames: [6, 7, 6, 7, 6, 7], fxH: 260, hitP: 0.3, twin: { gap: 30, spd: 1.4 } }, cd: 2, dmgMult: 3, aoe: true, rangePx: 200 },   // 1~5 히어로, 6·7 번개기둥 교차 반복
  ...
  { id: 39, n: 8, h: 200, stage: 4, title: '독검', charSeq: [1, 2, 3, 4, 5, 6, 8], cd: 2, dmgMult: 3, aoe: true, rangePx: 180, hitAt: 0.85 },   // 시트에 7번이 없어 번호를 그대로 씀(7은 미사용)
...
const SKILL_ICON_FRAME = { 1: 6, 2: 5, 7: 3, 8: 4, 13: 4, 15: 3, 16: 3, 17: 4, 18: 4, 20: 4, 22: 4, 23: 6, 24: 7, 25: 3, 26: 3, 27: 1, 28: 6, 29: 6, 31: 6, 32: 2, 33: 10, 34: 7, 35: 5, 36: 2, 37: 2, 38: 4, 39: 8, 40: 4, 41: 2, 42: 6, 43: 4, 44: 3, 45: 2, 46: 3, 47: 4, 48: 4 }
...
// 대륙 개방 순서 — 아프리카만 열려 있고, 앞 대륙을 10단계까지 다 깨야 다음이 열린다
const ADV_ORDER = ['africa', 'middle_east', 'europe', 'oceania', 'asia', 'greenland', 'south_america', 'north_america']
...
function mergeMotion(sv) {   // 저장된 모션값 + 기본값 병합 (초기 로드·클라우드 복원 공용)
  ...
  return {
    ...
    // MOT_FX_IDS(낙하/투사체 이펙트가 있는 스킬)만 돌리면, 이펙트가 없는 스킬의 연타값(skFx[id].tick)이
    // 병합에서 통째로 사라진다 — 편집기에서 잡아도 새로고침하면 없어지던 원인. 코드·저장본의 모든 id를 합친다.
    skFx: Object.fromEntries([...new Set([...MOT_FX_IDS.map(String), ...Object.keys(MOTION_DEFAULT.skFx), ...Object.keys(sv.skFx || {})])]
      .map(id => [id, { ...MOTION_DEFAULT.skFx[id], ...((sv.skFx || {})[id] || {}) }])),
  }
}
...
const SKILL_FRAME_T = {
  48: [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5],       // 궁극의 소용돌이 (확인용 0.5초)
  47: [0.5, 0.5, 0.5, 0.5, 0.5, 0.5],                // 회전검객 (확인용 0.5초, 조절 예정)
  46: [0.5, 0.5, 0.5, 0.5, 0.5],                     // 파워 찌르기 (확인용 0.5초, 조절 예정)
  45: [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5],   // 무한검술 (18프레임 × 0.5초 — 확인용, 조절 예정)
  44: [0.1, 0.1, 0.12, 0.1, 0.14],                   // 번개 토네이도 (히어로 5프레임)
  ...
  39: [0.08, 0.08, 0.08, 0.08, 0.08, 0.08, 0.25],   // 독검 (마지막 해골 폭발만 길게)
  37: [0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07],   // 순보 (11프레임)
...
const CARD_COST = { 1: 10, 10: 100 }          // 스킬 카드 소환 비용(다이아)
const ADV_OPEN_WAVE = 100                     // 모험 개방 — 최고 웨이브 도달 조건
const ALLY_OPEN_WAVE = 200                    // 동료 개방 — 최고 웨이브 도달 조건
// 진화 단계별 기본 해금 스킬(단계당 1개). 스킬 목록에서도 맨 앞에 온다
const SK_FREE = [7, 18, 24, 32, 38, 43]       // 0 할퀴기 · 1 바위치기(강화) · 2 암흑 강타 · 3 얼음도끼 · 4 난도질 · 5 피의 십자가
const skFreeInit = () => Object.fromEntries(SK_FREE.map(id => [id, 1]))
...
const EVOS = [
  // mult = 히어로 공격력 배수. 1/3/27/81/243/729 → 1/3/5/10/30/50 으로 축소(2026-08-12)
  // 밸런싱 모드 진화 조건은 히어로 레벨만 본다(lv). 고기(cost)는 디버그 모드 전용
...
export default function App() {
  ...
  // 대륙 잠금: 앞 대륙을 10단계까지 다 깨야 열린다(디버그 모드는 전부 개방)
  const advLocked = key => {
    if (!BAL) return false
    const i = ADV_ORDER.indexOf(key)
    if (i <= 0) return false
    const prev = ADV_ORDER[i - 1]
    return (advStage[prev] || 0) < ADV_STAGES
  }
  const advPrevName = key => {
    const i = ADV_ORDER.indexOf(key)
    return i > 0 ? (CONTINENTS.find(c => c.key === ADV_ORDER[i - 1]) || {}).name : ''
  }
  ...
  useEffect(() => {
    ...
    const wv = init.wave
    // 밸런싱 모드는 실제 전투와 같은 공식으로 — 옛 선형식을 쓰면 100웨이브에서 보상이 94배 적게 나온다
    const avgHp = BAL ? B.hp0 * B.hpR ** (wv - 1) : avg(types.map(t => t.hp)) * (1 + 0.4 * (wv - 1))
    const avgMeat = BAL ? B.meat0 * B.rwR ** (wv - 1) : avg(types.map(t => Math.floor(t.meat * (1 + 0.2 * (wv - 1)))))
    const avgExp = BAL ? B.expo0 * B.rwR ** (wv - 1) : avg(types.map(t => Math.floor(t.exp * (1 + 0.2 * (wv - 1)))))
    const st2 = S.current
    const dps = st2.atk * (1000 / st2.cd)
    // 마리당 처치+접근 시간. 상한·하한 없음 — 세면 많이, 약하면 적게(실제 전투와 일치).
    // min(6)이 있으면 못 잡는 웨이브에서도 6초에 한 마리를 쳐줘서 '높은 웨이브에 두는 게 항상 이득'이 된다.
    const killT = avgHp / Math.max(1, dps) + 1.2
  ...
  const bestRef = useRef(best); bestRef.current = best          // 랭킹 조회에서 최신값을 쓰기 위해
  ...
  useEffect(() => {
    ...
    function applySkillDmg(t, dmg, noStop, dot) {
      t.hp -= dmg
      // 독(지속 데미지): 시전이 끝나도 남아 간격마다 깎는다. 잡몹은 대개 즉사하니 사실상 보스용
      if (dot && dot.p > 0 && dot.dur > 0) t.dot = { d: dmg * dot.p / 100, iv: Math.max(0.05, dot.iv), left: dot.dur, t: 0 }
      t.flash = 1
    ...
    function loop(now) {
      ...
      if (st.phase === 'fighting') {
        ...
        for (const e of w.enemies) {
          if (e.dead) continue
          if (e.dot) {                                  // 독: 스킬이 끝난 뒤에도 계속 깎는다
            e.dot.left -= dt; e.dot.t += dt
            let _g = 0
          ...
          if (w.pull && (!e.boss || w.pull.boss) && e.x - w.heroX < w.pull.rng) {
            e.x += (w.pull.x - e.x) * Math.min(1, dt * w.pull.rate)   // 스르륵 빨려온다(순간이동 아님)
            e.lunge = 0; e.atkT = 0; e.atkHit = false
            e.animT += dt * SPEED
            e.flash = Math.max(0, e.flash - dt * 5)
            e.sq = Math.max(0, (e.sq || 0) - dt)
            continue
          }
          e.flash = Math.max(0, e.flash - dt * 5)
        ...
        if (w.skill == null) {
          ...
          if (sk.fx && sk.fx.type === 'strike' && !w.skillFx) {
            ...
            if (w.skillT >= _skCast * (_fxc.startP ?? sk.hitAt)) {
              ...
              w.strikes.push({ id: sk.id, frames: sk.fx.frames, x: x2, anchor: _fxc.anchor ? 1 : 0, twin: _tw, t: 0,
                ...
                tick: _fxc.tick ?? sk.fx.tick ?? 0, nextHit: 0,
                dot: { p: _fxc.dotP ?? 0, iv: _fxc.dotIv ?? 0.5, dur: _fxc.dotDur ?? 0 } })   // tick>0 이면 hitP 단발 대신 그 간격으로 계속 때린다
            }
          }
          ...
          const _fire = noStop => {
            ...
            const _dot = { p: _fc.dotP ?? 0, iv: _fc.dotIv ?? 0.5, dur: _fc.dotDur ?? 0 }
            if (sk.fx && sk.fx.type === 'proj') {
              ...
              for (const t of w.enemies) if (!t.dead && t.x - w.heroX < rng) { applySkillDmg(t, dmg, noStop, _dot); if (sk.stun) t.stun = sk.stun }
            } else {
              const targets = w.enemies.filter(e => !e.dead).sort((a, b) => a.x - b.x).slice(0, sk.maxTargets || 1)
              for (const t of targets) applySkillDmg(t, dmg, noStop, _dot)
            }
            if (!noStop) w.shake = 8
          }
          // 끌어당김: 시전 진행도 [pullFrom, pullTo] 구간 동안 사거리 안 적을 히어로 앞으로 빨아들인다
          const _pc = motRef.current.skFx[sk.id] || {}
          const _pp = _pc.pullPx ?? 0
          if (_pp > 0) {
            const _p0 = (_pc.pullFrom ?? 0.1) * _skCast, _p1 = (_pc.pullTo ?? 0.85) * _skCast
            w.pull = (w.skillT >= _p0 && w.skillT <= _p1)
              ? { x: w.heroX + _pp, rng: skEff(sk, st.skCfg).rangePx || 300, boss: (_pc.pullBoss ?? 0) > 0, rate: _pc.pullRate ?? 6 }
              : null
          } else w.pull = null
          ...
          if (sk.fx && sk.fx.type === 'strike') {
            const _fc = motRef.current.skFx[sk.id] || {}
            // noWait=1 이면 히어로 모션이 끝나는 즉시 시전 종료 → 바로 기본공격·걷기로 돌아간다.
            // 이펙트(w.strikes)는 시전과 무관한 객체라 그대로 재생되고 연타·데미지도 계속 들어간다.
            if (!(_fc.noWait > 0)) {
          ...
          if (w.skillT >= _castEnd) { w.skill = null; w.skillT = 0; w.pull = null }
        }
        // UI 동기화 (0.2초 간격)
        ...
        for (const stk of w.strikes) {
          ...
          const _hit = noStop => {
            if (!(stk.dmg > 0)) return
            const _dot = stk.dot
            if (stk.aoe) { for (const e of w.enemies) if (!e.dead && e.x - stk.hx < stk.rng) { applySkillDmg(e, stk.dmg, noStop, _dot); if (stk.stun) e.stun = stk.stun } }
            else { const e = w.enemies.find(e2 => !e2.dead && Math.abs(e2.x - stk.x) < 70); if (e) applySkillDmg(e, stk.dmg, noStop, _dot) }
          }
          if (stk.tick > 0) {
        ...
        if (hero.hp <= 0 && !w.adv && !w.ev) setPhase('gameover')
          ...
          if (w.bossBattle) {
            ...
            if (w.waveNum % 10 === 0) {
              ...
              if (!w.bossPrompted) { w.bossPrompted = true; setClearMsg(w.waveNum) }
              setBossReady(true)   // 반복할 때마다 다시 켠다
  ...
  const [rankInfo, setRankInfo] = useState(null)          // { me, total } — 최고 웨이브 기준 순위
  const refreshRank = async myBest => {
    if (!FB_ON || !fbAuth.currentUser) return
    try {
      // 문서를 받지 않고 개수만 센다(쿼리 2번). 내 등수 = 나보다 높은 사람 수 + 1
      const col = collection(fbDb, 'paleoRank')   // 세이브 본문과 분리 — 랭킹용 요약만 담긴 컬렉션
      const [tot, above] = await Promise.all([
        getCountFromServer(col),
        getCountFromServer(query(col, where('best', '>', myBest))),
      ])
      setRankInfo({ me: above.data().count + 1, total: tot.data().count })
    } catch (e) { setRankInfo(null); console.warn('[rank]', e.code || e.message) }
  }
  ...
  async function pushCloud(force) {
    ...
    try {
      ...
      if (!sv) return
      // 진행도가 크게 줄어드는 업로드는 막는다(저장소 초기화 사고 방지). 메뉴의 '클라우드에 저장'으로는 강제 가능
      const myBest = sv.best || sv.wave || 0
      if (!force && cloudBestRef.current > 5 && myBest < cloudBestRef.current * 0.5) {
        setCloudMsg(`업로드 보류 — 내 기록 ${myBest} < 클라우드 ${cloudBestRef.current}`)
        return
      }
      ...
      const payload = {
        ...
        ts: sv.ts || 0,
        wave: sv.wave || 0,
        best: sv.best || sv.wave || 0,   // 참고용(랭킹은 paleoRank 컬렉션으로 계산)
      }
      await setDoc(doc(fbDb, 'paleoSaves', fbAuth.currentUser.uid), payload)
      // 랭킹용 요약(최고 웨이브·닉네임)만 따로 — 이 컬렉션만 읽기를 열면 세이브 본문은 잠긴 채로 순위가 돈다
      await setDoc(doc(fbDb, 'paleoRank', fbAuth.currentUser.uid), { best: sv.best || sv.wave || 0, nick: sv.nick || '', ts: Date.now() })
      cloudBestRef.current = Math.max(cloudBestRef.current, myBest)
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(fbAuth, async u => {
      ...
      try {
        ...
        cloudBestRef.current = cloud ? (cloud.best || (cloudSave && cloudSave.best) || cloudWave || 0) : 0
        // 폰은 받기 전용: 클라우드와 다르면 무조건 클라우드를 받는다(폰 진행이 PC를 덮는 사고 차단)
        const cloudNewer = !IS_PC ? cloudTs !== bootTs
          : (cloudTs !== 0 || bootTs !== 0) ? cloudTs > bootTs : cloudWave > (local?.wave || 0)
  ...
  async function fbLogout() { await pushCloud(); await signOut(fbAuth) }   // PC가 아니면 pushCloud 가 알아서 건너뛴다
  ...
  function evolve() {
    ...
    if (BAL) {                                   // 밸런싱: 히어로 레벨만 본다(고기 소모 없음)
      if (hlv < (EVOS[evo + 1].lv || 0)) return
    } else {
  ...
  function retry() {
    const w = world.current
    w.bossBattle = false; w.startBossFlag = false; w.enemies = []
    setBossReady(wave % 10 === 0)   // 보스 웨이브면 바로 재도전 가능(웨이브 다시 안 잡아도 됨)
    w.needStart = true; setPhase('fighting')
  }
  function jumpWave(n) {
    ...
    <div ref={rootRef} style={{ ...st.root, width: BASE_W, maxWidth: 'none', height: view.h, flexShrink: 0, transform: `scale(${view.s})`, transformOrigin: 'top center' }} onClickCapture={e => {
          ...
          <div data-edit="menu" style={st.menuPanel} onClick={e => e.stopPropagation()}>
            ...
            {FB_ON && (fbUser ? (
              <>
                ...
                <button style={st.menuItem} onClick={() => pushCloud(true)}>클라우드에 저장 <span style={{ fontSize: 11, opacity: 0.6 }}>{cloudMsg}</span></button>
      ...
      <div className="pd-fade" ref={updFade} onScroll={e => updFade(e.currentTarget)} style={st.panelInner}>
        ...
        {tab === '진화' && (
          <div data-edit="row" style={st.row}>
            ...
            {evo < EVOS.length - 1
              ? <button data-edit="cost" style={{ ...st.costBtn, opacity: (BAL ? hlv >= (EVOS[evo + 1].lv || 0) : DEBUG || meat >= EVOS[evo + 1].cost) ? 1 : 0.4 }} onClick={evolve}>{BAL ? `Lv.${EVOS[evo + 1].lv}` : DEBUG ? '+1' : fmt(EVOS[evo + 1].cost)}</button>
      ...
      {nav === '스킬' && (
        <div data-edit="panel" style={st.panel}>
          ...
          <div style={st.skGrid}>
          {SKILLS.map((s, i) => [s, i])
            .filter(([s]) => s.stage === evo || (s.stages || []).includes(evo))
            .sort((a, b) => (SK_FREE.includes(b[0].id) ? 1 : 0) - (SK_FREE.includes(a[0].id) ? 1 : 0))   // 기본 해금 스킬을 맨 앞으로
            .map(([s, i]) => {
        ...
        <div style={st.advWrap}>
          <div style={st.advViewport}>
            <div ref={advTrackRef} style={{ ...st.advTrack, transform: `translateX(${advOffset}px)`, transition: advLoaded ? st.advTrack.transition : 'none', opacity: advLoaded ? 1 : 0 }}>
              ...
              {CONTINENTS.map((ct, i) => (
                <button key={ct.key} data-edit={`advbtn${i}`} style={{ ...st.advContBtn, left: `${ct.x}%`, top: `${ct.y}%`, transform: `translate(calc(-50% + var(--pd-advbtn${i}-x)), calc(-50% + var(--pd-advbtn${i}-y)))`, ...(advLocked(ct.key) ? { filter: 'grayscale(1) brightness(0.5)' } : null) }} onClick={() => { if (!uiEdit && !advLocked(ct.key)) setAdvSel(ct) }}>
                  {advLocked(ct.key) && <img src="/ui/lock.webp" alt="" data-edit="advlock" style={st.advLockIc} />}
      ...
      <div data-edit="nav" style={st.bottomNav}>
        {[['영웅', 'nav_hero'], ['스킬', 'nav_skill'], ['장비', 'nav_equip'], ['동료', 'nav_ally'], ['모험', 'nav_adventure'], ['상점', 'nav_shop']].map(([n, ic]) => {
          const need = n === '모험' ? ADV_OPEN_WAVE : n === '동료' ? ALLY_OPEN_WAVE : 0   // 탭 개방 웨이브
          const lk = BAL && need > 0 && best < need
          return (
          <button key={n} style={{ ...st.navBtn, ...(nav === n ? st.navActive : {}) }} onClick={() => { if (!lk) setNav(n) }}>
            <img src={`/icon/${ic}.webp`} alt="" style={{ ...st.navIconImg, ...(lk ? st.skLockedImg : null) }} />
            {lk && <img src="/ui/lock.webp" alt="" data-edit="navlock" style={st.navLockIc} />}
            <div style={{ fontSize: 'var(--pd-navfz)', ...(lk ? { opacity: 0.5 } : null) }}>{lk ? `${need}웨이브` : n}</div>
          </button>
          )
        })}
      </div>
      ...
      {IS_PC && motEdit && (() => {
        ...
        <div style={{ ...st.motPanel, ...(dockSide ? dockStyle : null) }}>
          ...
          {motCat === 'hero' && (<>
            ...
            {row('이펙트 안 기다림 (0/1)', (M.skFx[motHeroSk] || {}).noWait ?? 0, 0, 1, 1, v => setMotCfg({ ...M, skFx: { ...M.skFx, [motHeroSk]: { ...(M.skFx[motHeroSk] || {}), noWait: v } } }))}
            {(() => { const put4 = (k, v) => setMotCfg({ ...M, skFx: { ...M.skFx, [motHeroSk]: { ...(M.skFx[motHeroSk] || {}), [k]: v } } })
              const c4 = M.skFx[motHeroSk] || {}
              return (<>
                {row('끌어당김 거리(px) 0=없음', c4.pullPx ?? 0, 0, 300, 5, v => put4('pullPx', v))}
                {row('끌기 시작(시전 진행도)', c4.pullFrom ?? 0.1, 0, 1, 0.05, v => put4('pullFrom', v))}
                {row('끌기 끝(시전 진행도)', c4.pullTo ?? 0.85, 0, 1, 0.05, v => put4('pullTo', v))}
                {row('끌기 속도', c4.pullRate ?? 6, 1, 20, 0.5, v => put4('pullRate', v))}
                {row('보스도 끌기 (0/1)', c4.pullBoss ?? 0, 0, 1, 1, v => put4('pullBoss', v))}
            ...
            {(() => { const put3 = (k, v) => setMotCfg({ ...M, skFx: { ...M.skFx, [motHeroSk]: { ...(M.skFx[motHeroSk] || {}), [k]: v } } })
              return (<>
                {row('독 데미지(%) 0=없음', (M.skFx[motHeroSk] || {}).dotP ?? 0, 0, 100, 1, v => put3('dotP', v))}
                {row('독 간격(초)', (M.skFx[motHeroSk] || {}).dotIv ?? 0.5, 0.05, 3, 0.05, v => put3('dotIv', v))}
                {row('독 지속(초)', (M.skFx[motHeroSk] || {}).dotDur ?? 0, 0, 20, 0.5, v => put3('dotDur', v))}
                <div style={{ fontSize: 10, color: '#7b6a50', marginBottom: 6 }}>맞은 적에게 지속 데미지를 남깁니다. 시전이 끝나도 계속 깎이며, 한 틱은 그 타격 데미지의 독 데미지(%)입니다. 잡몹은 대개 즉사하니 사실상 보스용입니다</div>
              </>) })()}
...
const UI_DEFAULT = {
  ...
  navlock: 20, navlockX: 0, navlockY: 0,
  advlock: 24, advlockX: 0, advlockY: 0,
...
const EDIT_GROUPS = {
  ...
  rankpill: { label: '랭킹 표시', size: ['rankh', 'rankfz'], pos: 'rank' },
  ...
  navlock: { label: '탭 자물쇠', size: ['navlock'], pos: 'navlock' },
  advlock: { label: '대륙 자물쇠', size: ['advlock'], pos: 'advlock' },
...
const st = {
  ...
  advLockIc: {
    position: 'absolute', left: '50%', top: '50%',
    width: 'var(--pd-advlock)', height: 'var(--pd-advlock)',
    transform: 'translate(-50%, -50%) translate(var(--pd-advlock-x), var(--pd-advlock-y))',
    objectFit: 'contain', imageRendering: 'pixelated', pointerEvents: 'none', zIndex: 4,
  },
  navLockIc: {
    position: 'absolute', left: '50%', top: '38%',
    width: 'var(--pd-navlock)', height: 'var(--pd-navlock)',
    transform: 'translate(-50%, -50%) translate(var(--pd-navlock-x), var(--pd-navlock-y))',
    objectFit: 'contain', imageRendering: 'pixelated', pointerEvents: 'none',
  },
  ...
  rankPill: {                                               // 이벤트 던전 버튼 바로 아래 (우측 정렬)
    ...
    borderRadius: 7, border: '1px solid #6b4a24', background: 'rgba(20,13,7,0.82)',
    whiteSpace: 'nowrap', zIndex: 5,
  },
  rankK: { color: '#9c8a6c' },
  rankV: { color: GOLD, fontWeight: 700 },
  hpPill: {
...
Object.assign(UI_DEFAULT, {
  ...
  wbexitw: 48, wbexith: 22, wbexitfz: 11, wbexitX: 7, wbexitY: 70,   // 웨이브 보스전 나가기 — 모험 나가기와 같은 자리(둘은 동시에 안 뜬다)
